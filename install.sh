...
echo "  claude-guard        Inspect live sessions and reap only explicitly enabled conditions"
echo "  claude-disk         Inspect Claude disk usage and session logs"
echo "  claude-clean-disk   Purge Claude temp files when no sessions are active"
if [ "$DAEMON_CHOICE" = "a" ]; then
...
echo "  cat ~/.cc-reaper/logs/disk-monitor.log  View disk inspection log"
